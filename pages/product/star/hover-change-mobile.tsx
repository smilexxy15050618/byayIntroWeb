...
import { ISceneItem } from './hover-change';
...
const HoverChangeMobile: React.FC<{ data: ISceneItem[] }> = ({ data }) => {
  ...
  return (
    <Wrapper>
      ...
      <TabContentWrapper>
        {data[tabIndex].message.map(message => {
          return (
            <div>
              <SubTitle>{message.title}</SubTitle>
              {message.content}
            </div>
          );
        })}
        ...
        <LabelWrap>
          {
            data[tabIndex].label.map(labelName => <Label>{ labelName }</Label>)
          }
        </LabelWrap>
        <SubTitle>
          解决方案
...
export default HoverChangeMobile;
